...
_do:
	...
	if (x < y)
	{
		...
		cout << "" << "Weight of the Evaporating Dish + Filtrate Must be greater\n"
				 << setw(20) << "" << "than that the Weight of the Evaporating Dish on its own.\n";
	...
	gotoxy(20, 1);
	cout << "" << "TDS value for given sample is " << setw(10) << getTDS(x, y) << setw(20) << getQuality(getTDS(x, y)) << "\n";
	setData(getTDS(x, y));
	return;
}
...
string get_cur_time()
{
	...
	string date_time;
	date_time = itos(nowlocal.tm_mday) + " : " + itos(nowlocal.tm_mon) + " : " + itos(nowlocal.tm_year + 1900) + " : " + itos(nowlocal.tm_hour) + " : " + itos(nowlocal.tm_min) + " : " + itos(nowlocal.tm_sec);
	return date_time;
}